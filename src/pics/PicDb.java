...
public class PicDb {
  ...
  private static Connection getConnection() {
    try {
      ...
      DataSource source = (DataSource) env.lookup("jdbc/pics");
      return source.getConnection();
    } catch (Exception e) {
      System.out.println("Unable to get connection to the database. "+e.getMessage());
      e.printStackTrace();
    }
    return null;
  }
 ...
 private static String getTimestampStr(ResultSet rs, String colName) throws SQLException {
  Timestamp theVal = rs.getTimestamp(colName);
  if (theVal != null) {
...
//        the last row will be in the hashmap!
 public static HashMap getRow(String SQLText) throws MyException {
  ...
  Connection conn = getConnection();
  Statement stat = PicDb.getStatement(conn);
